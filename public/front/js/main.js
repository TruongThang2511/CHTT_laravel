...
function removeCart(rowId){
    $.ajax({
        ...
        success: function(response){
            ...
            var cartHover_tbody = $('.mini_cart');
            var cartHover_existItem = cartHover_tbody.find("i" + "[data-rowId'" + rowId + "']");

            cartHover_existItem.remove();
            var cart_tbody = $('.table-responsive tbody');
            var cart_existItem = cart_tbody.find("i" + "[data-rowId'" + rowId + "']");
...
function updateCart(rowId, qty){
    $.ajax({
        ...
        success: function(response){
            ...
            $('.select-total p').text('$' + response['total']);
            ...
            if(qty === 0){
                ...
                cartHover_existItem.find('.cart-price').text('$' + response['cart'].price.toFixed(2) + 'x' + response['cart'].qty);
            }
            ...
            if(qty === 0){
                ...
                cart_existItem.find('.product_total').text('$' + (response['cart'].price * response['cart'].qty).toFixed(2));
            }

            $('.cart_amount').text('$' + response['subtotal']);
            $('.cart_amount').text('$' + response['total']);
